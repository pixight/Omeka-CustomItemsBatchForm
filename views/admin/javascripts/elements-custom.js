...
(function ($) {
     ...
     */
    Omeka.ElementsCustom.elementFormRequest = function (fieldDiv, params, elementFormPartialUri, recordType, recordId) {
        ...
        //params.element_set = element_set;
        params.element_id = elementId;
        params.record_id = recordId;
        params.record_type = recordType;
        params.field_count = jQuery('.input-block',jQuery(fieldDiv)).length;
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                var pattern = /element_(\d+)/g;
                var replacement = 'custom_$1';
                responsetmp = response.replace(pattern, replacement);      

                console.log(responsetmp);
                
                pattern = /Elements\[/g;
                replacement = 'custom[';
                responsetmp = responsetmp.replace(pattern, replacement);
                pattern = /Elements-(\d+)/g;
     ...
     * @param {string} recordId Current record ID.
     */
    Omeka.ElementsCustom.makeElementControls = function (element, elementFormPartialUrl, recordType, recordId) {
        ...
        context.find(addSelector).click(function (event) {
            ...
            Omeka.ElementsCustom.elementFormRequest(fieldDiv, {add: '1'}, elementFormPartialUrl, recordType, recordId);
        });
     ...
     * @param {Element} element The element to search at and below.
     */
    Omeka.ElementsCustom.enableWysiwyg = function (element) {
